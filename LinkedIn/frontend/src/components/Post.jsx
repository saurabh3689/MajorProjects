...
function Post({ id, author, description, image, like, comment, createdAt }) {
   ...
   useEffect(()=>{
    ...
    return (
        <div className='w-full min-h-[200px] bg-white felx flex-col gap-[10px] rounded-lg shadow-lg p-[20px]'>
            <div className='flex items-center justify-between'>
                ...
                items-center justify-center curser-pointer' >
                    ...
                    <div>
                        <div className='text-[24px] font-bold'>{`${author.firstName} ${author.lastName}`}
                        </div>
                        <div className='text-[16px]'>{author.headline}
                        </div>
          ...
          <div className=' pl-[50px] text-[19px] font-semibold text-blue-500 cursor-pointer'
         ...
         <div>
            ...
            {showComment &&
            <div>
                <form className='w-full flex justify-between items-center border-b-2 
                border-b-gray-300 p-[10px]'
                onSubmit={handleComment}>
                    <input type="text" placeholder="leave a comment..." className='outline-none
                    border-none' value={commentContent} onChange={(e) => setCommentContent(e.target.value)} />
                    <button><LuSendHorizontal /></button>
                </form>
                <div className=' flex flex-col gap-[20px]'>
                    {comments.map((com, index) => (
                        <div key={index} className=' flex flex-col gap-[10px] border-b-2 p-[20px] border-b-gray-300'>
                          <div className='w-full flex justify-start items-center gap-[10px]'>
                            ...
                            <div>
                            <div className='text-[16px] font-semibold'>{`${com.user.firstName} ${com.user.lastName}`}
                             </div>

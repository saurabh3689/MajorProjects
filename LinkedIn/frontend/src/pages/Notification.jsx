...
function Notification() {
  ...
  return (
    <div className='w-screen h-[100vh] bg-bg-[#f0efe7] pt-[100px] px-[20px] flex flex-col
    items-center gap-[40px] '>
      <Nav />
      <div className='w-full h-[100px] bg-[white] shadow-lg rounded-lg flex items-center
       p-[10px] text-[22px] text-gray-600 justify-between'>
        <div>
        Notifications {notificationData.length}
      </div>
      {notificationData.length >0 &&
      <button className='min-w-[100px] h-[40px] rounded-full border-2 
            border-[#c50f0f] text-[#c50f0f]' onClick={handleClearAllNotification} >
              clear all</button>}
         </div>
      {notificationData.length > 0 &&
        <div className='w-[100%] max-w-[900px] bg-white shadow-lg rounded-lg flex flex-col gap-[20px] 
        h-[100vh] overflow-auto p-[20px] '>
          {notificationData.map((noti, index) => (
            <div className='w-full min-h-[100px] p-[20px] flex justify-between items-center
            border-b-2 border-b-gray-200' key={index}>
              <div>

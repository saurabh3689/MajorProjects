...
export const sendConnection = async (req, res) => {
    ...
    if(existingConnection){
        return res.status(400).json({message:"You have already sent connection request to this user"})
    }
    ...
    if(receiverSocketId){
        io.to(receiverSocketId).emit("statusUpdate",{updatedUserId:sender,
            newStatus:"received"})
    }
    if(senderSocketId){
        io.to(senderSocketId).emit("statusUpdate",{updatedUserId:id,
            newStatus:"pending"})
    }
...
export const acceptConnection = async (req, res) => {
    try{
        ...
        if(!connection){
            return res.status(404).json({message:"Connection request does not found"})
        }
...
await User.findByIdAndUpdate(connection.receiver, {
    $addToSet: { connection: connection.sender }
        ...
        if(receiverSocketId){
            io.to(receiverSocketId).emit("statusUpdate",{updatedUserId:connection.sender._id,
                newStatus:"disconnect"})
        }
        if(senderSocketId){
            io.to(senderSocketId).emit("statusUpdate",{updatedUserId:req.userId,
                newStatus:"disconnect"})
        }
...
export const removeConnection = async (req, res)=>{
    try{
        ...
        if(receiverSocketId){
            io.to(receiverSocketId).emit("statusUpdate",{updatedUserId:myId,
                newStatus:"connect"})
        }
        if(senderSocketId){
            io.to(senderSocketId).emit("statusUpdate",{updatedUserId:otherUserId,
                newStatus:"connect"})
        }
        return res.json({message:"Connection removed successfully"})
    }catch(error){
      return res.status(500).json({message: "removeConnectio error"})
    }
}
